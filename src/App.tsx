...
function AuthScreen({ onLogin, forcedRole }: { onLogin: (user: AuthUser, token: string) => boolean | void, forcedRole?: Role }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (accepted === false) {
        // Role mismatch â€” show generic error, don't reveal the real reason
                   ...
                   <div>
                     ...
                     <div className="grid grid-cols-2 gap-3 mt-4">
                       ...
                       <div className="bg-slate-50 rounded-xl p-3">
                         ...
                         <span className="font-mono font-black text-slate-600 text-sm">{zone.max_price ? `â‚µ${Number(zone.max_price).toFixed(2)}` : 'âˆž'}</span>
                       </div>
                     </div>
                   </div>
