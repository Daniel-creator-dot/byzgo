#!/usr/bin/env bash
# Release iOS build (same Maps + API setup as Android build_apk.sh).
set -euo pipefail
...
/// Auto-generated by scripts/build_ios.sh
class MapsKey {
  MapsKey._();
...
echo "BytzGo iOS — API_URL=$API_URL"
cd "$MOBILE_ROOT"
"$FLUTTER" pub get
"$FLUTTER" build ios --release --dart-define-from-file=dart_defines.json
echo ""
echo "Open ios/Runner.xcworkspace in Xcode to archive, or run: cd ios && xcodebuild ..."
